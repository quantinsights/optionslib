...
from optionslib.market.enums import FxOptionsMarketQuoteType
...
class VannaVolga:
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if not self.check_option_quotes_integrity():
            raise Exception(
                f"STDL, RR and FLY quotes must be present for each maturity!"
            )
    ...
    def unpack_option_quotes(self):
        ...
        for quote in enumerate(self.fx_option_market_quotes):
            if quote.quoteType == FxOptionsMarketQuoteType.ATM_STRADDLE:
    ...
    def check_option_quotes_integrity(self):
        ...
        for exp in self.__exp_dates:
            if (
                not (exp in self.__risk_rev)
                or (exp in self.__stdl)
                or (exp in self.__vwb)
            ):
                return False
