...
class ActualActual(DayCountBase):
    ...
    ACT/ACT assumes that a year consists of 365 or 366 days (in case of a leap year),
    and that the days between dates s and t, s prior to t, are counted as the actual
    number of calendar days between the two dates, including the first but not the
    second.

    If s and t are dates belonging to the same year and n is the actual number of days
    in the year, then the year fraction equals (t - s).

    If s and t are dates belonging to two different years, let J_i be the first of
    January of the second year, J_f be the first of January of the final year, n_i be
    the number of days in the first year, n_f be the number of days of the final year,
    and y the number of years between the first and the final year, then the year
